...
(function ($) {
	...
	var DTimePicker = function setTiming(elem, options){
		var that = this;
		this.input = $(elem);
		this.activeField = DTimePicker.HOUR;
		this.picker = null;

		this.hour = $('<input class="tp_hr" type="text" style="width:16px;" maxlength="2" name="tp_hr">');
	    ...
	    this.hour.on('change',
	        function () {
	            if (elem.id == "txt_starttime") {
	                var splitTime = $('#txt_starttime')[0].nextElementSibling.value;
	                var time1 = parseInt(splitTime) + 1;
	                if (time1 == 13)
	                    $('#txt_starttime')[0].nextElementSibling.nextElementSibling.nextElementSibling
                            .value = "PM";             


	                $('#txt_endtime')[0].nextElementSibling.value = (parseInt(time1) > 12 ? parseInt(time1) - 12 : parseInt(time1));
	                $('#txt_endtime')[0].nextElementSibling.nextElementSibling.nextElementSibling.value = (parseInt(time1) >12 ? "PM" : "AM");
	                that.setValue();
	                that.setEndTime();
	            }
	        });
	    this.min.on('change',
           function () {
               if (elem.id == "txt_starttime") {
                   $('#txt_endtime')[0].nextElementSibling.nextElementSibling.value = $('#txt_starttime')[0].nextElementSibling.nextElementSibling.value;
                   that.setValue();
                   that.setEndTime();
               }
           });
		...
		this.ampm.on('keydown', function (e) {
			if(e.keyCode == 80) { //P button
                ...
                //vikram
				if (elem.id == "txt_starttime") {
				    var splitTime = that.input["0"].value.split(':');
	                var time1 = parseInt(splitTime[0]) +1;
	                $('#txt_endtime')[0].nextElementSibling.value = (parseInt(time1) > 12 ? parseInt(time1) -12: parseInt(time1));
	                $('#txt_endtime')[0].nextElementSibling.nextElementSibling.value = splitTime[1];
	                $('#txt_endtime')[0].nextElementSibling.nextElementSibling.nextElementSibling.value = (parseInt(time1) > 12 ? "PM": "AM");
				    that.setEndTime();
				}
	...
	DTimePicker.prototype = {
	    ...
	    //vikram
		setEndTime: function () {
            var ap = $('#txt_endtime')[0].nextElementSibling.nextElementSibling.nextElementSibling.value;
            var h = $('#txt_endtime')[0].nextElementSibling.value < 10
                ? ("0" + $('#txt_endtime')[0].nextElementSibling.value)
                : $('#txt_endtime')[0].nextElementSibling.value;
            var m = $('#txt_endtime')[0].nextElementSibling.nextElementSibling.value;
            if (ap == "PM") h = parseInt(h) + 12;

            // if 12 pm(24:00) set as 12
            if (parseInt(h) == 24) {
                var val = 12 + ":" + m + ":00.000";
                $('#txt_endtime').attr('value', val);
            }
