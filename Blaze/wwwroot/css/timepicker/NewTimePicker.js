...
(function ($) {
    ...
    var DTimePicker = function setTiming(elem, options) {
        ...
        this.hour = $('<input class="tp_hr" type="text" style="width:43px;" maxlength="2" name="tp_hr">');
        this.min = $('<input class="tp_min" type="text" style="width:50px;" maxlength="2">');
        ...
        this.hour.on('change',
	        function () {
	            if (elem.id == "start0") {
	                var splitTime = $('#start0')[0].nextElementSibling.value;
	                var time1 = parseInt(splitTime) + 1;
	                if (time1 == 13)
	                    $('#start0')[0].nextElementSibling.nextElementSibling.nextElementSibling
                            .value = "PM";


	                $('#end0')[0].nextElementSibling.value = (parseInt(time1) > 12 ? parseInt(time1) - 12 : parseInt(time1));
	                $('#end0')[0].nextElementSibling.nextElementSibling.nextElementSibling.value = (parseInt(time1) > 12 ? "PM" : "AM");
	                that.setValue();
	                that.setEndTime();
	            }
	        });
        this.min.on('change',
           function () {
               if (elem.id == "start0") {
                   $('#end0')[0].nextElementSibling.nextElementSibling.value = $('#start0')[0].nextElementSibling.nextElementSibling.value;
                   that.setValue();
                   that.setEndTime();
               }
           });
        ...
        this.ampm.on('keydown', function (e) {
            if (e.keyCode == 80) { //P button
                ...
                if (elem.id == "start0") {
                    ...
                    $('#end0')[0].nextElementSibling.value = (parseInt(time1) > 12 ? parseInt(time1) - 12 : parseInt(time1));
                    $('#end0')[0].nextElementSibling.nextElementSibling.value = splitTime[1];
                    $('#end0')[0].nextElementSibling.nextElementSibling.nextElementSibling.value = (parseInt(time1) > 12 ? "PM" : "AM");
                    that.setEndTime();
                }
    ...
    DTimePicker.prototype = {
        ...
        //vikram
        setEndTime: function () {
            var ap = $('#end0')[0].nextElementSibling.nextElementSibling.nextElementSibling.value;
            var h = $('#end0')[0].nextElementSibling.value < 10
                ? ("0" + $('#end0')[0].nextElementSibling.value)
                : $('#end0')[0].nextElementSibling.value;
            var m = $('#end0')[0].nextElementSibling.nextElementSibling.value;
            if (ap == "PM") h = parseInt(h) + 12;
